...
#countries <- unique(as.character(gapminder$country[countries == seAsia]))
...
`m[2, c(4,5)]` will extract values 11 and 14 from matrix m.
...
`gapminder[1:20]` attempts to subset the first 20 columns of gapminder; however, gapminder only contains 6 rows and "can not subset past the end".

`gapminder[1:20, ]` attempts to subset the first 20 rows with all their columns.
...
```{r}
# solution 1
gapminder_small_a <- gapminder[1:9, ]
gapminder_small_b <- gapminder[19:23, ]
gapminder_small <- rbind(gapminder_small_a, gapminder_small_b)

#This is more efficient code version of solution above
